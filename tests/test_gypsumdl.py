...
def test_valid_data_population(tmp_path):
...
def test_invalid_keys(tmp_path):
    ...
    with pytest.raises(TypeError, match="must be a string or None"):
        GypsumDLConfig(str(cfg_path), data_dict=d)


def test_valid_bool_param(tmp_path):
...
def test_valid_int_param(tmp_path):
...
def test_valid_float_param(tmp_path):
...
def test_invalid_job_manager(tmp_path):
...
def test_invalid_num_processors(tmp_path):
...
def test_invalid_thoroughness(tmp_path):
...
def test_invalid_max_compound(tmp_path):
...
def test_ph_bounds_and_order(tmp_path):
    ...
    with pytest.raises(ValueError, match="min_ph must be <= max_ph"):
        GypsumDLConfig(str(cfg_path), data_dict=d_bad3)


def test_pka_precision_positive(tmp_path):
...
def test_mpijobs_processor_type(tmp_path):
...
def test_valid_mpijobs_processor(tmp_path):
...
def test_gypsum(tmp_path):
    ...
    config = GypsumDLConfig(str(tmp_path / "gypsum_config.yaml"), data_dict=d)
    ...
    test_output_file = os.path.join(
        outfolder, "gypsum_dl_success.sdf"
    )
    ...
    print(len(out_data))
    import numpy as np
    assert np.isclose(len(out_data), 50000, rtol=0.2)
