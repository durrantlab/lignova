...
def get_pdb_ids_from_parquet(
    ...
    if "PDB/Gene ID" in new_parquet.schema.names:
        ...
        logger.error(
            "The parquet file does not contain 'PDB/Gene ID' or 'Represenatives' column."
        )
        raise ValueError(
            "The parquet file does not contain 'PDB/Gene ID' or 'Represenatives' column."
        )
...
def extract_ligand_id(parquet_file: str, pdb_id: str) -> list[int]:
    ...
    member_compound_data = new_parquet.filter_data(
        condition=lambda x: x == pdb_id, column="Represenatives"
    )["member_compound"][0]
...
def make_smi_file_from_parquet(
    ...
    with open(output_smi, "w", encoding="utf-8") as smi_file:
        for ligand_id in ligand_source:
            smiles_data = new_parquet.filter_data(
                condition=lambda x: x == ligand_id, column="actives"
            )
            if (
                smiles_data["smiles"][0] is None
                or smiles_data["smiles"][0] == ""
                or smiles_data["pubmed"][0] is None
                or smiles_data["pubmed"][0] == np.nan
            ):
                logger.warning(
                    f"Missing smiles or pubmed data for ligand ID {ligand_id}. Skipping."
                )
                continue
            smi_file.write(
                f"{smiles_data['smiles'][0]} {smiles_data['actives'][0]}_{smiles_data['pubmed'][0]}\n"
            )
...
def add_pdb_ligand_to_smi(parquet_file: str, pdb_id: str, output_smi: str) -> None:
    ...
    filtered_data = new_parquet.filter_data(
        condition=lambda x: x == pdb_id, column="PDB/Gene ID"
    )
    ...
    ligands_data = new_parquet.filter_data(
        condition=lambda x: x == protein_cluster_number, column="Protein Cluster number"
    )
    if len(ligands_data) == 0:
        logger.warning(f"No ligands found for PDB ID {pdb_id} in the parquet file.")
        return
    logger.info(
        f"Adding {len(ligands_data)} ligands for PDB ID {pdb_id} to smiles file."
    )
...
def prep_proteins(
    ...
    if not os.path.exists(os.path.join(output_dir, temp_prot.file_id + "_cleaned.pdb")):
        ...
        logger.info(
            f"Cleaned protein file for {temp_prot.file_id}_cleaned.pdb already exists."
        )
    return pdb_file
...
def run_cli_prot(
    ...
    if not pdb_parq_source.endswith((".parquet", ".pq", ".pdb")):
        raise ValueError(
            "pdb_parq_source must be a parquet file with .parquet or .pq extension or a single pdb id with .pdb extension."
        )
    if pdb_parq_source.endswith(".parquet") or pdb_parq_source.endswith(".pq"):
        ...
        if num_proteins is not None:
            ...
            if not (
                isinstance(num_proteins, list)
                and len(num_proteins) == 2
                and all(isinstance(i, int) for i in num_proteins)
            ):
                raise ValueError(
                    "num_proteins must be a list of two integers representing the range of proteins to prepare."
                )
            # check if the end index is greater than the length of pdb_ids
            if num_proteins[1] > len(pdb_ids):
                logger.warning(
                    f"End index {num_proteins[1]} is greater than the number of available PDB IDs {len(pdb_ids)}. Adjusting to {len(pdb_ids)}."
                )
            ...
            logger.info(
                f"Preparing proteins in the range: {num_proteins[0]} to {num_proteins[1]}"
            )
        ...
        pdb_output_dir = os.path.join(
            output_dir, os.path.basename(pdb_parq_source).split(".pdb")[0]
        )
        prepped = prep_proteins(
            pdb_file=os.path.join(input_dir, pdb_parq_source), output_dir=pdb_output_dir
        )
        logger.info(f"Cleaned protein for {os.path.basename(pdb_parq_source)} is saved")
        return [prepped]
    ...
    for pdb_id in pdb_ids:
        ...
        # 3. prepare protein
        try:
            prepped = prep_proteins(pdb_file=pdb_filepath, output_dir=pdb_output_dir)
            prepared_prot_path.append(prepped)
            logger.info(f"Cleaned protein for {pdb_id} saved to {prepped}")
...
def make_mangable_files(file_list: list[str], split: int = 250) -> list[str]:
    ...
    for file in file_list:
        ...
        if len(lines) <= split:
            logger.info(
                f"The file {file} has {len(lines)} lines which is less than or equal to the split size of {split}. No splitting needed."
            )
...
def run_cli_ligand(
    ...
    if add_pdb_ligand:
        ...
        if not os.path.exists(pdb_ligand_parquet_source) or not os.path.isfile(
            clustered_parquet_source
        ):
            logger.error(
                f"The file {pdb_ligand_parquet_source} or {clustered_parquet_source} does not exist"
            )
    ...
    if pdb_id_source.endswith(".parquet") or pdb_id_source.endswith(".pq"):
        ...
        if num_proteins is not None:
            ...
            # Example: if num_proteins = [0, 50], prepare first 50 proteins
            logger.info(
                f"Preparing ligands for proteins in the range: {num_proteins[0]} to {num_proteins[1]}"
            )
        if not pdb_ids:
            raise RuntimeError("No PDB IDs found in parquet file.")
    else:
    ...
    for pdb_id in pdb_ids:
        ...
        if os.path.exists(pathfile):
            logger.info(
                f"Smiles file for {pdb_id} already exists at {pathfile}. Skipping generation."
            )
        ...
        if add_pdb_ligand:
            ...
            if os.path.exists(pathfile):
                ...
                logger.warning(
                    f"No ligands found for PDB ID {pdb_id} in the ligand source parquet file."
                )
            try:
            ...
            make_smi_file_from_parquet(
                ligand_source, extract_ligand_id(ligand_source, pdb_id), pathfile
            )
            if os.path.exists(pathfile):
                outputpaths.append(pathfile)
            else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help="Mode to run the script in: 'protein' to prepare proteins, 'ligand' to prepare ligands.",
    )

    parser.add_argument(
        "-i", "--input-dir", required=False, help="Input directory for raw PDB files."
    )
    parser.add_argument(
        "-p",
        "--parquet",
...
def main() -> None:
    ...
    if args.mode == "protein":
        if args.parquet.endswith((".parquet", ".pq")) and args.input_dir is None:
            parser.error(
                "The --input-dir argument is required when --parquet is a parquet file in protein mode."
            )
        ...
        output_file = os.path.join(
            args.output_dir, f"cleaned_pdblist_{args.start_index}_{args.end_index}.txt"
        )
        ...
        if args.ligand_source is None or args.output_smi is None:
            parser.error(
                "The --ligand-source and --output-smi arguments are required in ligand mode."
            )
        ...
        output_file = os.path.join(
            args.output_dir, f"smiles_filelist_{args.start_index}_{args.end_index}.txt"
        )
        with open(output_file, "w", encoding="utf-8") as f:
